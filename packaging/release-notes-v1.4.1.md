...
runs entirely on the endpoint. Nothing is uploaded, and the application makes
...
1. Download `LocalRCA-v1.4.1-windows-x64.zip` (433 MB).
2. **Verify it** — in PowerShell:
   ```powershell
   Get-FileHash LocalRCA-v1.4.1-windows-x64.zip -Algorithm SHA256
   ```
   Expected:
   ```
   6224D5EC2023AB8F30260A75289EB757DC2A15C2C7310611A6BC1746EF94C9A4
   ```
...
- **Structural contrast meets WCAG SC 1.4.11.** Text was always legible at
...
- **No update mechanism.** If a defect ships, it stays until you download a
  newer release yourself.
