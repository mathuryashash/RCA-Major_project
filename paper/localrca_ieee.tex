...
Bengaluru, India}
\and
\IEEEauthorblockN{Dr. Niharika P Kumar}
\IEEEauthorblockA{\textit{Associate Professor, Dept. of ISE} \\
\textit{RV Institute of Technology and Management}\\
Bengaluru, India}
}
...
that replays the pipeline over 596 incidents discovered in real collected
...
$106$ of $596$ incidents ($17.8\%$), with $36\%$ falling below the Granger
sample floor and therefore untestable at the default lag. We further show that
the hand-written subsystem prior rejects $22.7\%$ of statistically accepted
pairs and forbids mechanisms that demonstrably exist. Deploying the system to
non-specialist users imposed two further requirements, which we describe with
their limits: the learned model is encrypted at rest under the user's Windows
credentials, and the interface leads with one plain-language verdict per
screen while keeping every raw measurement one switch away. We argue that the
...
\begin{enumerate}
  ...
  \item A deployment design for a tool that holds sensitive telemetry---
        ...
        interface that separates the verdict from the evidence---together with
        the places it falls short (Section~\ref{sec:deploy}).
\end{enumerate}
...
microservice architectures and find that none dominates uniformly, that
performance is highly sensitive to the underlying call graph, and that
published recall numbers do not transfer cleanly between systems. Both
works assume a call graph or trace corpus that a single endpoint does not
have; the corresponding gap here is that our $596$-incident survey has no
comparable ground truth, so Section~\ref{sec:population} reports how often
the system speaks, not how often it is correct against a labelled corpus.
...
($L=5$, 30-day survey). The rate rises monotonically with window length,
confirming at scale the starvation effect first seen in a single pair of
injections.}
\label{fig:yield}
\end{figure}
...
is therefore $106/596 = 17.8\%$: roughly one incident in six receives a causal
explanation.

We report the funnel rather than the survivor-filtered rate. The share of
\emph{analysable} incidents explained is $27.8\%$, which is the more flattering
...
$2$ throughout. The gain is honest reporting---``tested, nothing
...
real---the Windows standby file cache expanding under write pressure---and is
the strongest single rejected relationship measured in this run, at $0.654$
...
where cycle-breaking already removes $14.7\%$ of pairs---so the two mechanisms
...
A research prototype can leave its data in plaintext and its results as raw
tables. A tool installed by someone else, holding a record of when and how
their machine is used, cannot. This section describes what the released
application (v1.5.1) does about both, and where it falls short.

\subsection{Protecting the model at rest}

The trained artefact holds the scaler's per-metric bounds and the per-metric
thresholds---a compact statistical profile of how the machine is used. It is
wrapped with the Windows Data Protection API (DPAPI), which binds the
ciphertext to the user's logon credentials, so a copy taken to another account
or machine cannot be read. The write is atomic, via a temporary file and a
rename, so an interrupted save cannot leave a truncated model. Artefacts from
earlier versions carry no header, still load, and are encrypted the next time
the model is trained. If the user's credentials are reset and the key is lost,
the application says the model must be retrained rather than surfacing a raw
Windows error. The database and logs, which DPAPI does not wrap, are covered by
a second, best-effort layer: at start-up the collector asks the Encrypting
File System (EFS) to encrypt its data folder.

Three findings qualify this. First, EFS does not exist on Windows Home
editions. The evaluation host runs one, the request fails on every start, and
its 227\,MB database remains in plaintext, protected only by file-system
permissions. The failure is logged rather than silent, but the protection a
user receives depends on which edition of Windows they bought. Second,
re-encryption happens only on retraining, so the model on the evaluation
host---trained before encryption was added---is still the plaintext artefact.
Third, independent review of the encryption change found that training wrote
an intermediate checkpoint in plaintext and deleted it only on success; an
interrupted run left unencrypted weights on disk. The checkpoint is now
removed in a \texttt{finally} block, and any leftover from an earlier crash is
purged when training starts.

\subsection{Signed binaries}

Both executables are Authenticode-signed with an RFC 3161 timestamp, and each
release publishes a SHA-256 checksum. The certificate is self-signed: it shows
that a binary is unchanged since it was built, not who built it, so Windows
SmartScreen still warns on first run. Removing the warning requires a
certificate from an authority Windows trusts, which this project does not
have.

\subsection{Separating the verdict from the evidence}

A review of the running application found that the first screen presented a
table of every captured channel with its raw latest value, and that training
readiness was expressed four different ways (clean samples, longest run,
current run, time remaining) that a non-specialist had to reconcile alone.
Hiding these would contradict the project's premise, since a user cannot
consent meaningfully to collection they cannot inspect.

The released interface therefore has two presentations of identical
computation, selected by a single global switch that persists across sessions
and is off by default. In the default view each tab leads with one sentence
whose colour encodes state: the data tab reports \emph{Healthy} only when
coverage is at least $90\%$ with at most three breaks, and \emph{Partial data}
from $50\%$; the training tab states whether a model can be trained and, if
not, how long remains. The inference tab keeps the incident picker and opens
on the plain-language report rather than the score table. The advanced view
restores the raw counts, the channel table, the database path, the training
hyperparameters, the Granger lag, and the score and outflow columns. Nothing is
computed differently in either view; the switch changes presentation only, and
the headline sentence is derived from the same coverage figures the advanced
view prints. The evaluation host itself reads \emph{Partial data} at $73\%$
coverage---accurate, if unflattering.

This design was checked by one expert review and by driving the live
application. It has not been measured with users, and whether the default view
helps a non-specialist reach a correct conclusion is unestablished.
...
incidents have no known cause, so a $17.8\%$ explanation rate reports how often
...
testing, which we believe generalises beyond this system. The pattern recurred
once encryption was added: two reviewers independently found the plaintext
training checkpoint of Section~\ref{sec:deploy} in code whose tests all
passed, because the tests exercised only the successful path. The suite now
stands at 150 tests.
...
cause is not the loudest metric, resolving whether the subsystem prior should
admit the feedback directions it currently forbids, encrypting the database
independently of EFS so that Home-edition users receive the same protection,
and measuring with users whether the default view leads them to correct
conclusions.
...
Int. Conf. on Automated Software Engineering (ASE)}, 2024.
